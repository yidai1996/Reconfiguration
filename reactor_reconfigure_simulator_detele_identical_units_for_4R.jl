...
function MPC_solve2(xBset,Tset,n,Flow,T0_inreal,T_0real,xA_0real,xB_0real,q_T,q_xA,q_xB,r_heat,r_flow,dt,P,N;heat_init=0,flow_init=0,print=true)
    ...
    for k=1:K
        ...
        # xB_tot_guess[k+1] = sum(n[i,New+1]*Flow[i,New+1]*xB_guess[i,k] for i=1:New)/sum(n[i,New+1]*Flow[i,New+1] for i=1:New)
        xB_tot_guess[k+1] = (3*Flow[1,New+1]*xB_guess[1,k+1]+Flow[2,New+1]*xB_guess[2,k+1])/(3*Flow[1,New+1]+Flow[2,New+1])
        # xB_tot_guess[k+1]=(3*Flow[1,3]*xB_guess[1,k+1]+Flow[2,3]*xB_guess[2,k+1])/(3*Flow[1,3]+Flow[2,3])
        # Tt_guess[k+1]=sum((n[i,N+1])*Flow[i,N+1]*T_guess[i,k] for i=1:N)/sum(n[i,N+1]*Flow[i,N+1] for i=1:N)
    end
...
function MPC_tracking(n1::Array{Int,2},n2,Dist_T0,SetChange_xB,SetChange_T,q_T,q_xA,q_xB,r_heat,r_flow,dt,P,
    ...
    global adjacentM2=zeros(N-1,N-1,time_steps+1)
    ...
    for tt=1:time_steps
        if tt>=setpoint_time+1
            ...
            heatvt[3,tt+1]=resultsheatvt[1]
            heatvt[4,tt+1]=resultsheatvt[2]

                flowvt[1,5,tt+1]=resultsflowvt[1,3]
                flowvt[2,5,tt+1]=resultsflowvt[1,3]
                flowvt[3,5,tt+1]=resultsflowvt[1,3]
                flowvt[4,5,tt+1]=resultsflowvt[2,3]
                flowvt[5,1,tt+1]=resultsflowvt[3,1]
                flowvt[5,2,tt+1]=resultsflowvt[3,1]
                flowvt[5,3,tt+1]=resultsflowvt[3,1]
                flowvt[5,4,tt+1]=resultsflowvt[3,2]
        else
        resultsheatvt,resultsflowvt=MPC_solve(xBsetpoint[:,tt],Tsetpoint[:,tt],adjacentM1[:,:,tt],flowvt[:,:,tt],T0_invt[:,tt],Tvt[:,tt],xAvt[:,tt],xBvt[:,tt],q_T,q_xA,q_xB,r_heat,r_flow,dt,P,N;heat_init=heatvt[1,tt],flow_init=flowvt[1,1,tt],print=print)
            for i=1:N
